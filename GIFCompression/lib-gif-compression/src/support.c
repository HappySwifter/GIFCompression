...
parse_frame_spec(Clp_Parser *clp, const char *arg, int complain, void *thunk)
{
  ...
  if (isdigit(c[0]))
    frame_spec_1 = frame_spec_2 = (int)strtol(c, &c, 10);
  else if (c[0] == '-' && isdigit(c[1]))
    frame_spec_1 = frame_spec_2 = Gif_ImageCount(input) + (int)strtol(c, &c, 10);
  ...
  if (c[0] == '-' && (frame_spec_2 >= 0 || c[1] != 0)) {
    c++;
    if (isdigit(c[0]))
      frame_spec_2 = (int)strtol(c, &c, 10);
    else if (c[0] == '-' && isdigit(c[1]))
      frame_spec_2 = Gif_ImageCount(input) + (int)strtol(c, &c, 10);
    else
...
parse_dimensions(Clp_Parser *clp, const char *arg, int complain, void *thunk)
{
  ...
  if (*arg == '_' && arg[1] == 'x') {
    ...
    dimensions_x = (int)strtol(arg, &val, 10);
  if (*val == 'x') {
    if (val[1] == '_' && val[2] == 0) {
      dimensions_y = 0;
      val = val + 2;
    } else
      dimensions_y = (int)strtol(val + 1, &val, 10);
    if (*val == 0)
      return 1;
  }
...
parse_position(Clp_Parser *clp, const char *arg, int complain, void *thunk)
{
  ...
  position_x = (int)strtol(arg, &val, 10);
  if (*val == ',') {
    position_y = (int)strtol(val + 1, &val, 10);
    if (*val == 0)
      return 1;
  }
...
parse_rectangle(Clp_Parser *clp, const char *arg, int complain, void *thunk)
{
  ...
  char *val;
  int x = position_x = (int)strtol(arg, &val, 10);
  (void)thunk;

  if (*val == ',') {
    int y = position_y = (int)strtol(val + 1, &val, 10);
...
parse_color(Clp_Parser *clp, const char *arg, int complain, void *thunk)
{
  ...
  if (*++str == 0) goto error;
  green = (int)strtol(str, &str, 10);
  if (*str != ',' && *str != '/') goto error;

  if (*++str == 0) goto error;
  blue = (int)strtol(str, &str, 10);
  if (*str != 0) goto error;
